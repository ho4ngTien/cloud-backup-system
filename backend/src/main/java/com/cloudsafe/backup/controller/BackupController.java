package com.cloudsafe.backup.controller;
...
import com.cloudsafe.backup.service.BackupService;
...
public class BackupController {
    ...
    @GetMapping("/latest-completed-files")
    @Operation(summary = "Get files and key from the latest completed backup version of a device")
    public ResponseEntity<Map<String, Object>> latestCompletedFiles(
            @AuthenticationPrincipal UserDetails principal,
            @RequestParam String deviceId) {
        return ResponseEntity.ok(backupService.getLatestCompletedFilesAndKey(principal.getUsername(), deviceId));
    }

    @GetMapping("/pending")
    @Operation(summary = "Get pending backup jobs for a device")
    public ResponseEntity<List<BackupJobDto>> getPendingJobs(
            @AuthenticationPrincipal UserDetails principal,
            @RequestParam String deviceId) {
        return ResponseEntity.ok(backupService.getPendingJobs(principal.getUsername(), deviceId));
    }
}
